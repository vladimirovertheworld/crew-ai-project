...
if ! git push -u origin main; then
    handle_warning "Push to 'main' branch failed. Attempting to push to 'master' branch..."
    if ! git push -u origin master; then
        handle_warning "Push to 'master' branch also failed. You may need to:"
        echo "1. Ensure you have the correct permissions for the repository."
        echo "2. Check if your SSH key is added to your GitHub account."
        echo "3. Manually push when ready using 'git push -u origin main' or 'git push -u origin master'"
        if prompt_user "Do you want to open the GitHub repository page to check for issues?"; then
            if command_exists xdg-open; then
                xdg-open $repo_url
            elif command_exists open; then
                open $repo_url
            else
                echo "Please manually open: $repo_url"
            fi
        fi
    else
